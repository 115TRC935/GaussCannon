...
class VisualizadorGraficos:
    ...
    def agregar_controles(self):
        ...
        self.ax_prev = self.fig.add_axes([0.2, 0.03, 0.1, 0.05])
        self.ax_next = self.fig.add_axes([0.7, 0.03, 0.1, 0.05])
    ...
    def crear_fig_vueltas(self):
        ...
        ax.set_xlabel("n° Vueltas", fontsize=14)
        ax.set_ylabel("Velocidad final máxima (m/s)", fontsize=14)
        ax.set_title("Envolvente superior: Velocidad máxima vs n° vueltas", fontsize=16)
        ax.grid(True)
    ...
    def crear_fig_superficie(self):
      ...
      Xi, Yi, Vi = superficie_maxima_cacheada(self.vueltas_np, self.d_hilo_np * 1000, self.v_finales_np, res=150, cache_file="superficie_vf_cache.npz")
      _, _, Ri = superficie_maxima_cacheada(self.vueltas_np, self.d_hilo_np * 1000, self.resistencia_np, res=150, cache_file="superficie_r_cache.npz")
      _, _, Li = superficie_maxima_cacheada(self.vueltas_np, self.d_hilo_np * 1000, self.l_bobina_np, res=150, cache_file="superficie_l_cache.npz")
      ...
      mappable.set_array([])
      cbar = self.fig.colorbar(mappable, ax=ax3d, pad=0.1, label="Resistencia [Ω]")
