...
csv_path = "resultados_simulacion.csv"
...
if os.path.exists(csv_path):
    ...
    vueltas = cp.arange(10, 801, 5, dtype=cp.float32)           
    ...
    # --- Cálculo realista de bobinado ---
        ...
        for k in range(1, n_steps):
            ...
            if k == 1:
                # tipo_amortiguamiento = cp.where(delta < -d0, 0,      # subamortiguado
                #                        cp.where(delta > d0, 2, 1))   # sodio
